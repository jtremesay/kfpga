...
from amaranth.lib.data import Const as DataConst
...
def _prepare_value(dut_field, value):
    ...
    if isinstance(value, int) and isinstance(dut_field.shape(), ArrayLayout):
        ...
        inner_size = (
            inner_shape.size
            if isinstance(inner_shape, ArrayLayout)
            else inner_shape.width
        )
        if inner_size == 1:
            ...
            return [
                [
                    (((value >> (i * inner_size)) & inner_mask) >> j) & 1
...
async def assertTestCase(
    ...
    for field in test_expected.__dataclass_fields__:
        ...
        actual_value = (
            raw_value.as_bits() if isinstance(raw_value, DataConst) else raw_value
        )
