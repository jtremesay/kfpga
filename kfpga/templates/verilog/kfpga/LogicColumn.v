...
{%- for y in range(module.height) %}
    {% if y < module.height - 1 %}wire [{{ module.interconnect_pairs_count - 1 }}:0] lt{{ y }}_north_out;{% endif %}
    {% if y > 0 %}wire [{{ module.interconnect_pairs_count - 1 }}:0] lt{{ y }}_south_out;{% endif %}
    LogicTile lt{{ y }}(
        .data_north_in({% if y == module.height -1 %}data_north_in{% else %}lt{{ y + 1 }}_south_out{% endif %}),
